  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const openItems = submissions.filter((s) => s.status === 'new' || s.status === 'discussion_needed' || s.status === 'questions' || s.status === 'in_progress' || s.status === 'ready_for_testing').length;
